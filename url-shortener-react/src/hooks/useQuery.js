...
export const useFetchTotalClicks = (token, onError) => {
    return useQuery("url-totalclick",
         async () => {
          ...
          {
            select: (data) => {//uswd for transformation,..
                ...
                const convertToArray = Object.keys(data.data).map((key) => ({
                    clickDate: key,
                    count: data.data[key], // data.data[2024-01-01], .get(key) it is returning the value
                }));
            ...
            onError,
            staleTime: 5000//for how much time the data will be inside cache,..
          }
        );
};